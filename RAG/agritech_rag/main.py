...
from rag_pipeline import DocumentProcessor, RAGChatbot, VectorDatabase

app = FastAPI(title="AgriTech AI Advisor API")
  ...
  try:
    ...
    if not file.filename.lower().endswith(allowed_extensions):
      raise HTTPException(
          status_code=400,
          detail=(
              "Chỉ hỗ trợ các định dạng tài liệu: .txt, .md, .pdf, .docx"
          ),
      )
    ...
    try:
      ...
      raise HTTPException(
          status_code=400,
          detail=f"Không thể đọc nội dung từ file: {str(err)}",
      )
    ...
    return {
        "status": "success",
        "message": f"Đã nạp thành công tài liệu '{file.filename}' vào cơ sở dữ liệu!",
